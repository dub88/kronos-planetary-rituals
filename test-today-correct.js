...
async function calculatePlanetaryHours(latitude, longitude, date, timezone = 'America/Denver') {
  ...
  try {
    ...
    for (let hourNumber = 1; hourNumber <= 24; hourNumber++) {
      ...
      if (hourNumber === 1) {
        // First hour is ruled by the day ruler
        planetName = dayRuler;
      } else {
        // Find the position of the day ruler in the sequence
        const dayRulerIndex = PLANETARY_HOUR_SEQUENCE.indexOf(dayRuler);
        
        // Calculate the offset for this hour
        const hourOffset = hourNumber - 1; // 0-based index
        
        // Get the planet for this hour following the Chaldean order
        const sequencePosition = (dayRulerIndex + hourOffset) % 7;
        planetName = PLANETARY_HOUR_SEQUENCE[sequencePosition];
      }
